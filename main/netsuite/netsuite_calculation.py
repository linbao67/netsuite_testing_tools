...
class NetsuiteClient(object):
    ...
    def convert_credit_memo_to_csv(self):
        ...
        for record in records:
            ...
            if 'discountTotal' in record:
                ...
                trans_discount_total = discountTotal
                is_pre_tax = df_item[df_item['internalId'] == int(discountItem_internalId)]['isPreTax'].values[0]

            flag_refund_prefix = False
            if 'applyList' in record:
                records = record['applyList']['apply']
                if isinstance(records, list):
                    for item in records:
                        if item.get('apply', 'false').lower() == 'true' and item.get('type', 'other') == 'Customer Refund':
                           flag_refund_prefix = True
                           break

                else:
                    if records.get('apply', 'false').lower() == 'true' and records.get('type', 'other') == \
                            'Customer Refund':
                        flag_refund_prefix = True
                    ...
                    if 'item' in line:
                        ...
                        if item_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem', 'PaymentItem']:
                                cal_sub_total = cal_sub_total + float(line.get('amount',0))
                    else:
                        # in case there is no item name for the line
                        cal_sub_total = cal_sub_total + float(line.get('amount'))

            # calculate the transaction level discount, if there is no transaction level discount, discount rate is
            # 1, off rate is 0, will not check in after logic
            trans_discount_rate = record.get('discountRate')
            if trans_discount_rate is not None and trans_discount_rate[-1] == '%':
                cal_trans_discount_rate = 1 - -1 * float(trans_discount_rate[:-1]) * 0.01
            elif trans_discount_rate is not None and trans_discount_rate != 0.0:
                cal_trans_discount_rate = 1 - -1 * trans_discount_total / cal_sub_total
            else:
                cal_trans_discount_rate = 1

            cal_trans_off_rate = (1 - cal_trans_discount_rate) * -1

            print("cal_trans_discount_rate {} cal_trans_off_rate {}".format(str(cal_trans_discount_rate),
                                                                            str(cal_trans_off_rate)))

            # here is to get the line level actual off rate
            if 'itemList' in record:
                item_lines = record['itemList']['item']
                ...
                for line in item_lines:
                    ...
                    if 'item' in line:
                        ...
                        if item_id != '0':
                            ...
                            print("items line no: {} item type {} amount {}".format(line_no, item_type,
                                                                                    line_amount))

                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem', 'DiscountItem']:
                                # item_list_name[line_no] = 'Refund - {}'.format(name) if flag_refund_prefix else name
                                item_list_name[line_no] = name

                            if item_type == 'DiscountItem':

                                if 'isTaxable' in line:
                                    if line['isTaxable'].upper() == 'TRUE':
                                        tmp_tax_rate = invoice_tax_rate
                                    else:
                                        tmp_tax_rate = 0

                                elif 'tax1Amt' in line:
                                    tmp_tax_amount = float(line['tax1Amt'])
                                    tmp_tax_rate = tmp_tax_amount / line_amount * 100 if tmp_tax_amount != 0.0 else 0

                                else:
                                    tmp_tax_rate = float(line.get('taxRate1', 0))
                                    tmp_tax_amount = line_amount * tmp_tax_rate

                                if last_discountable_item:
                                    item_list_discount.append(line_no)

                                    if 'rate' in line and line['rate'][-1] == '%':
                                        tmp_off_rate = float(line['rate'][:-1]) * 0.01
                                        tmp_amount = round(last_discountable_item['sum'] * tmp_off_rate, 0)
                                        if tmp_amount != round(line_amount, 0):
                                            tmp_off_rate = line_amount / last_discountable_item['sum']
                                    elif line_amount != 0.0:
                                        tmp_off_rate = line_amount / last_discountable_item['sum']
                                    else:
                                        tmp_off_rate = 0
                                    tmp_cal_tax_rate = tmp_tax_amount / last_discountable_item['sum'] if \
                                        tmp_tax_amount != 0.0 else 0
                                    tmp_discount_item = last_discountable_item.copy()
                                    tmp_discount_item['off_rate'] = tmp_off_rate
                                    tmp_discount_item['tax_rate'] = tmp_cal_tax_rate
                                    tmp_discount_item['line_amount'] = line_amount
                                    tmp_discount_item['tax_amount'] = tmp_tax_amount

                                    item_list_discount_apply_to[line_no] = tmp_discount_item

                                else:
                                    last_markupable_item = {'start': line_no, 'end': line_no,
                                                            'sum': line_amount}

                                    item_list_name[line_no] = name
                                    item_list_published_discounts.append(line_no)
                                    tmp_discount_item = {'off_rate': tmp_off_rate, 'tax_rate': tmp_tax_rate,
                                                         'sum': line_amount, 'tax_amount': tmp_tax_amount}

                                    item_list_discount_apply_to[line_no] = tmp_discount_item

                                if flag_inside_itemgroup:
                                    if not flag_subtotal_start_in_grp:
                                        previous_subtotal_item_in_grp += 1
                                else:
                                    if not flag_subtotal_start:
                                        previous_subtotal_item_out_grp += 1

                            elif item_type == 'SubtotalItem':
                                end_subtotal_item = line_no - 1
                                    ...
                                    if flag_inside_itemgroup:
                                        if flag_discountable_in_grp_inc_markup:
                                            pos_subtotal_items = {'start': previous_subtotal_item_in_grp + 1,
                                                                  ...
                                                                  'sum': line_amount,
                                                                  'type': item_type}
                                            last_discountable_item = pos_subtotal_items.copy()
                                        else:
                                            last_discountable_item = None
                                        if not flag_subtotal_start_in_grp:
                                            previous_subtotal_item_in_grp += 1
                                        ...
                                        if not flag_subtotal_start:
                                            previous_subtotal_item_out_grp += 1
                                last_markupable_item = last_discountable_item.copy() if last_discountable_item else None
                                ...
                                if flag_inside_itemgroup:
                                    ...
                                    flag_subtotal_start_in_grp = False
                                    previous_subtotal_item_in_grp = line_no

                                else:
                                    flag_discountable_inc_markup = True
                                    flag_discountable_exc_markup = True
                                    flag_subtotal_start = False
                                    previous_subtotal_item_out_grp = line_no
                                ...
                                flag_discountable_in_grp_inc_markup = False
                                flag_discountable_in_grp_exc_markup = False
                                start_group_item = line_no + 1
                                last_discountable_item = None
                                last_markupable_item = None
                            elif item_type == 'MarkupItem':
                                print("Inside Markup Item, last markup {}".format(str(last_markupable_item)))
                                if last_markupable_item:
                                    item_list_markup_apply_to[line_no] = last_markupable_item
                                    item_list_markup.append(line_no)
                                else:
                                    item_list_unknown_markup.append(line_no)
                                    item_list_name[line_no] = 'Markup on unknown item'
                                ...
                                if flag_inside_itemgroup:
                                    ...
                                    if not flag_discountable_in_grp_exc_markup:
                                        last_markupable_item = {'start': line_no, 'end': line_no,
                                                                'sum': line_amount}
                                        if line_amount != 0.0:
                                            last_discountable_item = {'start': line_no, 'end': line_no,
                                                                      'sum': line_amount}
                                        else:
                                            last_discountable_item = None
                                else:
                                    ...
                                    if not flag_subtotal_start:
                                        previous_subtotal_item_out_grp += 1
                                    if not flag_discountable_exc_markup:
                                        last_markupable_item = {'start': line_no, 'end': line_no,
                                                                'sum': line_amount}
                                        if line_amount != 0.0:
                                            last_discountable_item = {'start': line_no, 'end': line_no,
                            ...
                            if end_group_item < start_group_item:
                                ...
                                pos_group_items = {'start': start_group_item, 'end': end_group_item,
                                                   'sum': line_amount, 'type': 'ItemGroupEnd'}

                            last_discountable_item = pos_group_items.copy()
                            last_markupable_item = last_discountable_item.copy()
                            flag_inside_itemgroup = False
                    else:
                        item_list_name[line_no] = ''
                        # in case there is no item name for the line
                        cal_sub_total = cal_sub_total + float(line.get('amount'))
                        previous_item_type = 'InventoryItem'

                        last_discountable_item = {'start': line_no, 'end': line_no, 'line_amount':
                            line_amount, 'type': previous_item_type, 'sum': 0}
            ...
            print("Credit Memo {} published discount {}".format(credit_note_number, str(item_list_published_discounts)))

            item_list_off_rate = dict.fromkeys(item_list_name.keys(), 0.0)
            item_list_tax_rate = dict.fromkeys(item_list_name.keys(), 0.0)
            print("Credit Memo {} dicount apply to {}".format(credit_note_number, str(item_list_discount_apply_to)))

            item_list_discount.reverse()
            item_list_discount_to_discount = {}

            # calculate each discount's actual discount rate, due to discounts applying on
            print(item_list_discount)
            for j in item_list_discount:
                discount = item_list_discount_apply_to[j]

                tmp_off_rate = discount['off_rate'] * item_list_discount_to_discount.get(j, 1)
                tmp_total_amount = discount['sum']
                tmp_tax_amount = discount['tax_amount']
                start = discount['start']
                end = discount['end'] + 1
                for i in range(start, end):
                    if i in item_list_discount:
                        item_list_discount_to_discount[i] = item_list_discount_to_discount.get(i,
                                                                                               1) + tmp_off_rate
                        item_list_discount_apply_to[i]['tax_amount'] = item_list_discount_apply_to[i]['tax_amount'] \
                            + tmp_tax_amount * item_list_discount_apply_to[i]['line_amount'] / tmp_total_amount

            print('after calculation {}'.format(str(item_list_discount_apply_to)))
            print(str(item_list_discount_to_discount))

            item_list_discount.reverse()
            for line_no in item_list_discount:
                discount = item_list_discount_apply_to[line_no]
                tmp_line_off_rate = discount['off_rate']
                tmp_total_amount = discount['sum']
                tmp_tax_amount = discount['tax_amount']
                start = discount['start']
                end = discount['end'] + 1
                ...
                for i in range(start, end):
                    item_list_off_rate[i] = item_list_off_rate.get(i,
                                                                   0) + tmp_line_off_rate * item_list_discount_to_discount.get(
                        line_no, 1)
                    if tmp_tax_amount != 0:
                        item_list_tax_rate[i] = item_list_tax_rate.get(i, 0) + tmp_tax_amount / tmp_total_amount

            print("credit memo {} discount rate {}".format(credit_note_number, str(item_list_off_rate)))
            print("credit memo {} tax rate {}".format(credit_note_number, str(item_list_tax_rate)))

            print("item name list {}".format(str(item_list_name)))
            ...
            for markup in item_list_markup:
                ...
                name = ''
                for i in range(start, end):
                    if i in item_list_name:
                        name = name + ', ' + item_list_name[i]
                name = 'Markup on ' + name[2:]
                print(name)

                item_list_name[markup] = name
                ...
                if 'item' in item_line:
                    ...
                    if item_id != '0':
                        ...
                        if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem', 'DiscountItem'] or \
                                item_type == 'DiscountItem' and line_no in item_list_published_discounts:
                            if item_type == 'MarkupItem':
                                name = item_list_name[line_no]
                            if flag_refund_prefix:
                                name = "Refund-{}".format(name)
                            description = name
                            quantity = float(item_line['quantity']) if 'quantity' in item_line else None
                            rate = item_line.get('rate')
                            amount = float(item_line.get('amount',0))
                            ...
                            if item_type == 'DiscountItem':
                                tgt_discount = -1 * amount * item_list_discount_to_discount.get(line_no,
                                                                                                1) * cal_trans_discount_rate
                            elif item_type == 'PaymentItem':
                                tgt_discount = -1 * amount * item_list_off_rate.get(line_no, 0)

                            else:
                                tgt_discount = -1 * amount * (item_list_off_rate.get(line_no, 0)
                                                              * cal_trans_discount_rate + cal_trans_off_rate)
                            print("line no {} discount {}".format(line_no,tgt_discount))
                            ...
                            if is_pre_tax:
                                if item_type == 'DiscountItem':
                                    tgt_tax_amount = tax_amount * cal_trans_discount_rate

                                else:
                                    tgt_tax_amount = tax_amount * cal_trans_discount_rate + amount * \
                                                     item_list_tax_rate.get(line_no, 0) * cal_trans_discount_rate
                            else:
                                if item_type == 'DiscountItem':
                                    tgt_tax_amount = tax_amount

                                else:
                                    tgt_tax_amount = tax_amount + amount * item_list_tax_rate.get(line_no, 0)
                            ...
                            credit_memo_lines.append(dict(credit_memo_id=credit_memo_id,
                                                          ...
                                                          cal_discount_rate=cal_trans_discount_rate,
                                                          tgt_tax_amount=tgt_tax_amount,
                                                          updated_at=updated_at))
                else:
                    description = "Refund-" if flag_refund_prefix else ""
                    quantity = float(item_line['quantity']) if 'quantity' in item_line else None
                    rate = item_line.get('rate')
            ...
            if 'shippingCost' in record:
                ...
                if flag_refund_prefix:
                    shipMethod_name = "Refund-{}".format(shipMethod_name)
            ...
            if 'handlingCost' in record:
                description = 'Refund-handling cost' if flag_refund_prefix else 'handling cost'
    ...
    def convert_cash_refund_to_csv(self):

        df_currency = pd.read_csv(os.path.join(get_csv_path(), ep.CURRENCY + '_' + str(self.batch_id) + '.csv'))
        df_item = pd.read_csv(os.path.join(get_csv_path(), 'item_ref_' + str(self.batch_id) + '.csv'))
        records = self.read_json_files(ep.CASH_REFUND, self.batch_id)
        credit_memo = []
        credit_memo_lines = []
        for record in records:
            credit_memo_id = record.get('internalId', '')
                    ...
                    if 'item' in line:
                        ...
                        if item_id != '0':
                            ...
                            if item_type == 'DiscountItem':
                                ...
                                if last_discountable_item:
                                    print('last_discountable item : {}'.format(str(last_discountable_item)))
                                    item_list_discount.append(line_no)
                            ...
                            if end_group_item < start_group_item:
                                ...
                                last_discountable_item = pos_group_items.copy()
                                last_markupable_item = pos_group_items.copy()
                            flag_inside_itemgroup = False
                    else:
                        item_list_name[line_no] = ''
                        # in case there is no item name for the line
                        ...
                        last_markupable_item = {'start': line_no, 'end': line_no, 'line_amount':
                            line_amount, 'type': previous_item_type, 'sum':0}
        ...
        df_creditMemo.to_csv(raw_csv_path,
                             ...
                             header=['internalId', 'tranId', 'total', 'tranDate',
                                     'currency/internalId',
                                     'exchangeRate', 'createdDate', 'lastModifiedDate', 'currency'])
        df_credit_memo_line['tgt_description'] = df_credit_memo_line['tgt_description'].map(lambda x: 'Refund-{}'.format(x))

        df_credit_memo_line.to_csv(line_csv_path,mode="a+", header=False)
        ...
        for record in records:
            ...
            if 'applyList' in record:
                ...
                if isinstance(records, list):
                    ...
                    if records.get('apply', 'false').lower() == 'true' and records.get('type', 'other').lower() == \
                            'invoice':
                        doc = records['doc']
                        line = records['line']
                        amount = records['amount']
    ...
    def convert_invoice_to_csv(self):
        ...
        df_customer_payment_apply = df_customer_payment_apply[pd.to_datetime(df_customer_payment_apply.updated_at) < datetime.utcnow()]
        df_credit_memo_apply = pd.read_csv(
            os.path.join(get_csv_path(), 'creditMemo_apply_' + str(self.batch_id) + '.csv'))
        df_credit_memo_apply = df_credit_memo_apply[
            pd.to_datetime(df_credit_memo_apply.updated_at) < datetime.utcnow()]
        ...
        for record in records:
            ...
            trans_discount_total = float(record.get('discountTotal', 0.0))
                        ...
                        if item_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem']:
                                if item_type == 'PaymentItem':
                                    ...
                                    cal_sub_total = cal_sub_total + float(line.get('amount'))
                                if item_type == 'DiscountItem':
                                    cal_sum_discount = cal_sum_discount + float(line.get('amount',0))
                    else:
                        cal_sub_total = cal_sub_total + float(line.get('amount'))
            ...
            if 'itemCostDiscount' in record:
                ...
                print("item cost tax rate: {}".format(item_cost_discount_tax))
            ...
            if 'expCostDiscount' in record:
                ...
                if exp_cost_discount_item_type == 'DiscountItem':
                    cal_sum_discount = cal_sum_discount + exp_cost_discount_amount
            ...
            if 'timeDiscount' in record:
                ...
                time_discount_amount = float(record.get('timeDiscAmount', 0))
                ...
                if time_discount_item_type == 'DiscountItem':
                    cal_sum_discount = cal_sum_discount + time_discount_amount

                if 'timeTaxRate1' in record:
                    time_discount_tax_rate = float(record.get('timeTaxRate1'))
                    time_discount_tax = time_discount_amount * time_discount_tax_rate
                else:
                    time_discount_tax = 0
            ...
            if 'timeList' in record:
                ...
                time_list_name = time_list_name[2:]
                print('time list total {}'.format(time_list_total))
                    ...
                    if apply:
                        ...
                        if item_internal_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem']:
                                ...
                                if item_type not in ['DiscountItem','MarkupItem']:
                                    item_cost_list_name = item_cost_list_name + ', ' + item_name
                item_cost_list_name = item_cost_list_name[2:]
            ...
            if 'expCostList' in record:
                ...
                exp_cost_list_name = exp_cost_list_name[2:]
            ...
            if 'itemList' in record:
                ...
                for line in item_lines:
                    ...
                    if 'item' in line:
                        ...
                        if item_id != '0':
                            ...
                            if item_type == 'DiscountItem':
                                ...
                                if last_discountable_item:
                                    ...
                                    item_list_published_discounts.append(line_no)
                                    tmp_discount_item['off_rate'] = tmp_off_rate
                                    tmp_discount_item['tax_rate'] = tmp_tax_rate
                                    tmp_discount_item['line_amount'] = line_amount
                                    tmp_discount_item['tax_amount'] = tmp_tax_amount
                        ...
                        last_discountable_item = {'start': line_no, 'end': line_no, 'sum':
                            line_amount, 'type': previous_item_type}
            ...
            for j in item_list_discount:
                ...
                for i in range(start, end):
                    if i in item_list_discount:
                        ...
                        item_list_discount_apply_to[i]['tax_amount'] = item_list_discount_apply_to[i]['tax_amount'] \
                                                                       + tmp_tax_amount * \
                                                                       item_list_discount_apply_to[i][
                                                                           'line_amount'] / tmp_total_amount
            ...
            print("invoice {} discount rate {}".format(invoice_id, str(item_list_off_rate)))
                    ...
                    if 'item' in item_line:
                        ...
                        if item_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem', 'DiscountItem'] or \
                                ...
                                # Todo don't need to calculate tax rate of item itself, can get tax amount dirrectly,
                                #  otherwise when we encount the amount == 0 and tax1amount > 0, we will have issue!
                                if 'isTaxable' in item_line:
                                    if item_line['isTaxable'].upper() == 'TRUE':
                                        tax_rate_1 = invoice_tax_rate
                                    else:
                        ...
                        tgt_discount = -1 * amount * round(item_list_off_rate.get(line_no, 0), 2)
                        ...
                        tax_rate_1 = tax_rate_1 + item_list_tax_rate.get(line_no, 0)

                        tgt_tax_amount = amount * cal_trans_discount_rate * tax_rate_1 * 0.01
            ...
            if 'timeList' in record:
                ...
                for time_line in time_list:
                    ...
                    if time_line['apply'].upper() == 'TRUE':
                        ...
                        if is_pre_tax:
                            if time_discount_item_type == 'DiscountItem':
                                tgt_discount = -1 * amount * (time_discount_amount / time_list_total *
                                                              cal_trans_discount_rate + cal_trans_off_rate)
                            else:
                                tgt_discount = -1 * amount * cal_trans_off_rate
                        else:
                            if time_discount_item_type == 'DiscountItem':
                                tgt_discount = -1 * amount * (time_discount_amount / time_list_total)
                        ...
                        if 'tax1Amt' in time_line:
                            tgt_tax_amount = float(time_line['tax1Amt'])
                        else:
                            tax_rate_1 = float(time_line.get('taxRate1', 0))
                            tgt_tax_amount = amount * tax_rate_1

                        if time_discount_item_type == 'DiscountItem':
                            tgt_tax_amount = tgt_tax_amount + time_discount_tax * amount / time_list_total
                            print("time list {} {} {} {}".format(tgt_tax_amount,time_discount_tax, amount, time_list_total))

                        if str(is_pre_tax).upper() == 'TRUE':
                            print('isPreTax {} is True'.format(is_pre_tax))
                            tgt_tax_amount = tgt_tax_amount * cal_trans_discount_rate
                    ...
                    if apply:
                        ...
                        if item_internal_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem', 'DiscountItem',
                                ...
                                if item_cost_discount_item_type == 'DiscountItem':
                                    tgt_discount = -1 * amount * (item_cost_discount_amount / item_cost_list_total *
                                                                  cal_trans_discount_rate + cal_trans_off_rate)
                                else:
                                    tgt_discount = -1 * amount * cal_trans_off_rate
                                ...
                                if 'tax1Amt' in item_cost_line:
                                    tgt_tax_amount = float(item_cost_line['tax1Amt'])
                                else:
                                    tax_rate_1 = float(item_cost_line.get('taxRate1', 0))
                                    tgt_tax_amount = tax_rate_1 * tax_rate_1

                                if item_cost_discount_item_type == 'DiscountItem':
                                    tgt_tax_amount = tgt_tax_amount + item_cost_discount_tax * amount / item_cost_list_total
                                    print("item cost: {} {} {} {}".format(item_cost_discount_tax,amount,
                                                                       item_cost_list_total,tgt_tax_amount))
            ...
            if 'expCostList' in record:
                ...
                for exp_cost in exp_cost_list:
                    ...
                    if exp_cost['apply'].upper() == 'TRUE':
                        ...
                        tgt_unit_price = amount
                        if exp_cost_discount_item_type == 'DiscountItem':
                            tgt_discount = -1 * amount * (exp_cost_discount_amount / exp_cost_list_total *
                                                          cal_trans_discount_rate + cal_trans_off_rate)
                        else:
                            tgt_discount = -1 * amount * cal_trans_off_rate
                        ...
                        if 'tax1Amt' in exp_cost:
                            tgt_tax_amount = float(exp_cost['tax1Amt'])
                        else:
                            tax_rate_1 = float(exp_cost.get('taxRate1', 0))
                            tgt_tax_amount = tax_rate_1 * tax_rate_1

                        if exp_cost_discount_item_type == 'DiscountItem':
                            tgt_tax_amount = tgt_tax_amount + exp_cost_discount_tax * amount / exp_cost_list_total
            ...
            if item_cost_discount_item_type == 'MarkupItem':
                ...
                if str(is_pre_tax).upper() == 'TRUE':
                    print('isPreTax {} is True'.format(is_pre_tax))
                    tgt_tax_amount = item_cost_discount_amount * cal_trans_discount_rate * item_cost_tax_rate * 0.01
                else:
            ...
            if exp_cost_discount_item_type == 'MarkupItem':
                ...
                if str(is_pre_tax).upper() == 'TRUE':
                    print('isPreTax {} is True'.format(is_pre_tax))
                    tgt_tax_amount = exp_cost_discount_amount * cal_trans_discount_rate * exp_cost_tax_rate * 0.01
                else:
            ...
            if time_discount_item_type == 'MarkupItem':
                ...
                if str(is_pre_tax).upper() == 'TRUE':
                    print('isPreTax {} is True'.format(is_pre_tax))
                    tgt_tax_amount = time_discount_amount * cal_trans_discount_rate * time_tax_rate * 0.01
                else:
            ...
            tgt_invoice_discount_total = (cal_sum_discount + trans_discount_total) * -1
            print("invoice discount {} {}".format(trans_discount_total, cal_sum_discount))
            ...
            invoice.append(dict(id=invoice_id,
                                ...
                                src_discountTotal=trans_discount_total,
