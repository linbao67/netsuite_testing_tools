...
DATE_FORMAT = '%Y-%m-%dT%H:%M:%s.000-%Z'# 2019-09-01T00:00:00.000-07:00
class NetsuiteClient(object):
    ...
    def convert_credit_memo_to_csv(self):
        ...
        for record in records:
            ...
            if 'discountItem' in record:
                ...
                tgt_quantity = 1
                tgt_unit_price = 0
                tgt_discount = abs(discountTotal)
                tgt_net_total = 0
                tgt_tax_amount = 0
                        ...
                        if item_id != '0':
                            ...
                            if item_type not in ['ItemGroup', 'DescriptionItem', 'SubtotalItem']:
                                ...
                                if discount_rate is not None and discount_rate[-1] == '%':
                                    cal_discount_rate = 1 - abs(float(discount_rate[:-1])) * 0.01
                                elif discount_rate is not None and cal_sub_total != 0.0:
                                    cal_discount_rate = 1 - abs(discountTotal) / cal_sub_total
                                else:
                                    cal_discount_rate = 1
    ...
    def convert_invoice_to_csv(self):
        ...
        for record in records:
            ...
            invoice_number = record.get('tranId', '')
            ...
            if due_date is None:
                if terms:
                    if terms.startswith('Net'):
                        no_of_days = int(terms[4:])
                    else:
                        no_of_days = 0
                    tgt_due_date = datetime.strptime(date, DATE_FORMAT) + timedelta(days=no_of_days)
                else:
                    tgt_due_date = datetime.strptime(date, DATE_FORMAT) + timedelta(days=30)
            ...
            discount_total = abs(float(record.get('discountTotal', 0.0)))
            ...
            if 'itemCostList' in record:
                ...
                for item_cost_line in item_cost_lines:
                    ...
                    apply = item_cost_line['apply'] == 'TRUE'  # make sure apply exists in itemCost
                    if apply:
                        doc = item_cost_line.get('doc')
                        line = item_cost_line.get('line')
                        df_tmp_item = df_item[df_vendor_bill['internalId'] == int(doc) and df_vendor_bill['line'] ==
                                              int(line)]
            ...
            if discount_rate is not None and discount_rate[-1] == '%':
                ...
                cal_discount_rate = 1 - abs(discount_total) / cal_sub_total
            else:
                cal_discount_rate = 1
                ...
                for time_line in time_list:
                    ...
                    if time_line['apply'] == 'TRUE':
                        ...
                        if 'taxCode' in time_line:
                            ...
                            tax_code = None
                            print(
                                'Credit Memo {} Item {} doesnt have tax_code'.format(invoice_number,
                                                                                     item_id))
            ...
            if 'expCostList' in record:
                ...
                for exp_cost in exp_cost_list:
                    ...
                    if exp_cost['apply'] == 'TRUE':
                        ...
                        tax_code = time_line['taxCode'].get('name') if 'taxCode' in exp_cost else None
            ...
            if 'itemCostDiscount' in record:
                ...
                if item_type == 'DiscountItem':
                    tgt_discount = item_cost_discount_amount
                    cal_sum_discount = cal_sum_discount + tgt_discount
                elif item_type == 'MarkupItem':
                    tgt_discount = 0
                ...
                item_cost_tax_rate = record.get('itemCostTaxRate1')
            ...
            if 'expCostDiscount' in record:
                ...
                if item_type == 'DiscountItem':
                    tgt_discount = abs(exp_cost_discount_amount)
                    cal_sum_discount = cal_sum_discount + tgt_discount
                elif item_type == 'MarkupItem':
                    tgt_discount = 0
                ...
                exp_cost_tax_rate = record.get('expCostTaxRate1')
            ...
            if 'timeDiscount' in record:
                ...
                if item_type == 'DiscountItem':
                    tgt_discount = abs(time_discount_amount)
                    cal_sum_discount = cal_sum_discount + tgt_discount
                elif item_type == 'MarkupItem':
                    tgt_discount = 0
                ...
                time_tax_code = record['timeTaxCode'][
                    'name'] if 'timeTaxCode' in record else None  #

                time_tax_rate = record.get('timeTaxRate1')
            ...
            tgt_invoice_discount_total = discount_total + cal_sum_discount
